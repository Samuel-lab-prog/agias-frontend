...
export function useSubjectDetails() {
	const { enrollmentId, classOfferingId } = useParams<{
		enrollmentId: string;
		classOfferingId: string;
	}>();
	const dashboardQuery = useMyStudentDashboard();
	const parsedEnrollmentId = Number(classOfferingId ?? enrollmentId);
	const hasValidId = Number.isInteger(parsedEnrollmentId) && parsedEnrollmentId > 0;
	const enrollment = hasValidId
		? dashboardQuery.dashboard?.enrollments.find(
				(item) => (classOfferingId ? item.classOffering.id : item.id) === parsedEnrollmentId,
			)
		: undefined;
