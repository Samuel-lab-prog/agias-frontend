	...
	return (
		<VStack align='stretch' gap={5}>
			<AdminHeader
				...
				description='Vincule um docente para disponibilizar a turma no espaço do professor.'
				action={<AdminLink to='/admin'>Visão geral</AdminLink>}
			/>
			<AdminState query={query} empty={query.data?.length === 0}>
							...
							<HStack justify='space-between' flexWrap='wrap' gap={3}>
								<Box>
									<Text fontSize='xs' color='fg.muted'>
										{item.code} · {item.academicPeriod.code}
									</Text>
									<Text fontWeight='semibold' mt={1}>
										{item.title}
									</Text>
								</Box>
								<AdminLink to={`/staff/classes/${item.id}`}>Gerenciar vínculo</AdminLink>
							</HStack>
						</Surface>
					))}
				</VStack>
