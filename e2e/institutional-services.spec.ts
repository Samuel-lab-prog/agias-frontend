...
const context = {
	...
	email: 'marina@instituicao.example',
	avatarUrl: null,
	campusId: 1,
	campus: {
		id: 1,
		...
		institution: { id: 1, name: 'Instituição de demonstração', acronym: 'DEMO', configured: true },
	},
	staffProfile: { id: 9, department: { name: 'Registros acadêmicos', code: 'RA' } },
};
const doc = {
	id: '56a1cad0-397d-4e68-9bf1-77aaf11f1c28',
	kind: 'enrollment',
	createdAt: '2026-09-12T15:00:00Z',
	verificationCode: 'a'.repeat(48),
	revokedAt: null,
...
async function setup(page: Page, role = 'staff') {
	...
	await page.route('**/institution/context', (route) =>
		route.fulfill({ json: { ...context, role } }),
	);
	await page.route('**/curriculum/academic-periods', (route) => route.fulfill({ json: [] }));
}
test('project search waits for an explicit search before showing results', async ({ page }) => {
	await setup(page);
	let listRequests = 0;
	const project = {
		id: 7,
		...
		kind: 'research',
		origin: 'internal',
		startsAt: '2026-08-01T03:00:00Z',
		endsAt: '2026-09-02T02:59:59Z',
		status: 'draft',
		finalReportStatus: 'not_submitted',
		coordinatorId: 100,
		coordinator: { id: 100, name: context.name },
		department: null,
		_count: { participants: 0, reports: 0 },
	};
	await page.route('**/projects/**', async (route) => {
		if (route.request().resourceType() === 'document') return route.continue();
		const path = new URL(route.request().url()).pathname.split('/projects')[1];
		if (path === '/departments' || path === '/catalogs') return route.fulfill({ json: [] });
		if (path === '/' && route.request().method() === 'GET') {
			listRequests += 1;
			return route.fulfill({ json: { items: [project], total: 1, page: 1, pageSize: 20 } });
		}
	...
	await expect(page.getByRole('heading', { name: project.title })).toHaveCount(0);
	expect(listRequests).toBe(0);
	await page.getByRole('button', { name: 'Buscar', exact: true }).click();
	await expect(page.getByRole('heading', { name: project.title })).toBeVisible();
	expect(listRequests).toBe(1);
});
test('staff profile shows readable identity and persistent navigation on desktop and mobile', async ({
	page,
}) => {
	await setup(page);
	...
	await expect(
		page.getByRole('link', { name: 'Projetos', exact: true }).filter({ visible: true }),
	).toBeVisible();
...
test('admin can open team details, permissions and campus configuration', async ({ page }) => {
	...
	const user = {
		...
		staffProfile: { id: 9, departmentId: 1, department: { name: 'Registros acadêmicos' } },
		studentProfile: null,
	};
	await page.route('**/admin/users?*', (r) =>
		r.fulfill({ json: { items: [user], total: 1, page: 1, pageSize: 20 } }),
	);
	await page.route('**/admin/users/100', (r) => r.fulfill({ json: user }));
	await page.route('**/admin/departments', (r) =>
		r.fulfill({ json: [{ id: 1, name: 'Registros acadêmicos' }] }),
	);
	await page.route('**/admin/permissions', (r) =>
		r.request().resourceType() === 'document' ? r.continue() : r.fulfill({ json: [] }),
	);
	...
	await page.goto('/staff/my-profile');
	await expect(
		page.getByRole('link', { name: 'Equipe', exact: true }).filter({ visible: true }),
	).toBeVisible();
});
test('professor creates proposal and adds a participant with work plan', async ({ page }) => {
	...
	await page.route('**/projects/**', async (r) => {
		...
		const path = new URL(r.request().url()).pathname.split('/projects')[1],
			method = r.request().method();
		if (path === '/departments' || path === '/catalogs') return r.fulfill({ json: [] });
		if (path === '/candidates')
			return r.fulfill({ json: [{ id: 8, name: 'Ana Souza', role: 'student' }] });
		if (path === '/' && method === 'POST') {
			project = { ...project, ...r.request().postDataJSON() };
			return r.fulfill({ json: project, status: 201 });
		}
		if (path === '/7/participants') {
			project.participants.push({
				id: 1,
				...r.request().postDataJSON(),
				approvedHours: null,
				user: { id: 8, name: 'Ana Souza' },
			});
			return r.fulfill({ json: { id: 1 } });
		}
	...
	await page
		.getByLabel('Plano de trabalho')
		.fill('Preparar atividades e conduzir oficinas de robótica.');
...
test('student emits a document and verification works without login', async ({ page }) => {
	...
	await page.route('**/documents/', (r) => {
		if (r.request().method() === 'POST') {
			issued = true;
			return r.fulfill({ json: doc, status: 201 });
		}
		return r.fulfill({ json: issued ? [doc] : [] });
	});
	await page.route('**/documents/participations', (r) => r.fulfill({ json: [] }));
	await page.route('**/documents/verify/*', (r) =>
		r.fulfill({
			json: {
				id: doc.id,
				title: doc.snapshot.title,
				subjectName: 'Ana Souza',
				institution: doc.snapshot.institution,
				campus: doc.snapshot.campus,
	...
	await expect(page.getByRole('link', { name: 'Baixar PDF' }).first()).toHaveAttribute(
		'href',
		new RegExp(doc.id + '/pdf$'),
	);
